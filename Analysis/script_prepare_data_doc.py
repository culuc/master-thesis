...
for i in range(1,l_in):
    ...
    # %% save result
    print(sys.argv[l_in-1+(i-1)*3])
    term1_topN_bySpeakerParty.to_csv(sys.argv[l_in-1+(i-1)*3])
    term1_topN_bySpeakerParty_scaled.to_csv(sys.argv[l_in+(i-1)*3])
    term1_topN_bySpeakerParty_share.to_csv(sys.argv[l_in+1+(i-1)*3])

    # %% save interim results
    term1_tf.to_pickle(sys.argv[l_in+2+(i-1)*3])
    term1topN_tf.to_csv(sys.argv[l_in+3+(i-1)*3])
